...
class CustomerApiApplicationTests {
	...
	void testCreateCustomer_Success() {
		CustomerResponseDTO response = customerService.createCustomer(testCustomerRequest);
	...
	void testUpdateCustomer_Success() {
		CustomerResponseDTO created = customerService.createCustomer(testCustomerRequest);
		...
		CustomerResponseDTO updated = customerService.updateCustomer(created.getId(), updateRequest);
	...
	void testSearchCustomers_NoResults() {
		...
		List<CustomerResponseDTO> results = customerService.searchCustomers("NonExistentName");
	...
	void testGetCustomersByStatus() {
		...
		List<CustomerResponseDTO> activeCustomers = customerService.getCustomersByStatus(CustomerStatus.ACTIVE);

		assertNotNull(activeCustomers);
		assertTrue(activeCustomers.size() > 0);
		assertTrue(activeCustomers.stream().allMatch(c -> "ACTIVE".equals(c.getStatus())));
	}
